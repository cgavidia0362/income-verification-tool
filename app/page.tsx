      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center text-white mb-8">
          ...
          <p className="text-xl opacity-90">
            Upload bank statements and get instant income analysis
          </p>
        </div>
              ...
              >
                ...
                <p className="text-gray-600 text-lg">
                  {selectedFile
                    ...
                    : 'or click to browse (PDF, Word, Images supported)'}
                </p>
                <input
                  type="file"
                  id="fileInput"
          ...
          {isProcessing && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 mt-3">
                Extracting transactions and categorizing income sources
              </p>
            </div>
          )}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                <div className="bg-gradient-to-br from-purple-600 to-purple-800 text-white p-8 rounded-2xl shadow-lg">
                  <h3 className="text-sm uppercase tracking-wider opacity-90 mb-3">
                    Total Income
                  </h3>
                  <div className="text-4xl font-bold mb-2">
                    $
                    {results.totalIncome.toLocaleString('en-US', {
                      minimumFractionDigits: 2,
                    })}
                  </div>
                  <div className="text-sm opacity-80">
                    {results.months.length} months analyzed
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-purple-600 to-purple-800 text-white p-8 rounded-2xl shadow-lg">
                  ...
                  <div className="text-4xl font-bold mb-2">
                    $
                    {(
                      results.totalIncome / results.months.length
              ...
              {/* Monthly Breakdown */}
              {results.months.map((month, idx) => (
                <div
                  key={idx}
                  className="bg-gray-50 rounded-2xl p-8 mb-6 border-l-4 border-purple-600"
                >
                  {/* Month Header */}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    {Object.entries(month.categories).map(
                      ([catName, catData]) => (
                        <div
                          key={catName}
                          className="bg-white p-5 rounded-xl border-l-4 border-purple-600 shadow-sm"
                        >
                          ...
                          <div className="text-2xl font-bold text-gray-800">
                            $
                            {catData.amount.toLocaleString('en-US', {
                              minimumFractionDigits: 2,
                            })}
                          </div>
                          <div className="text-xs text-gray-400 mt-1">
                            {catData.count} transaction
                            {catData.count > 1 ? 's' : ''}
                          </div>
                        </div>
                      )
                        ...
                        {month.transactions.map((tx, txIdx) => (
                          <tr key={txIdx} className="border-b hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4">
                              <span
                                className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${getTransactionTypeClass(
                                  tx.type
                                )}`}
                              >
                                {tx.type}
